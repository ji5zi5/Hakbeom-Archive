...
배경은 `public/assets/bg/` 아래 generated VN background bundle을 사용한다. 각 배경은 같은 이름의 `.prompt.txt`를 같이 둔다. 이 번들은 GitHub `agent-sprite-forge`의 repo-local `generate2dmap` 설치를 기준으로 관리하며, VN 장면 배경은 SVG가 아니라 PNG baked raster로 둔다. route-depth 배경은 manifest의 `routeId` + `expansionBatch` 또는 `dayN-<route>-<beat>` 파일명으로 deterministic route binding을 유지한다. 장면 분위기가 바뀌면 `BCG` directive로 배경을 명시하고, 모든 장면을 `/assets/ui/image0_13_6.jpg` 하나에 몰아넣지 않는다.
...
- Generated background는 PNG, `.prompt.txt`, `agent-sprite-forge-manifest.json` `sourceGeneratedImage` provenance, scenario `BCG` reference가 한 세트다. route-depth 배경은 `routeId`와 asset provenance `expansionBatch: route-depth-2026-05` metadata도 유지한다. Scene story batch(`route-depth-2026-05-batch2` 등)는 scenario flow 구분용이고 asset provenance batch와 같은 값일 필요는 없다. SVG, 단색 배경, `derivedFrom`/색보정 파생 배경으로 대체하지 않는다.
...
전면 보이스 리라이트는 `src/data/routeConfig.js`의 `datingSimProfiles.*.voiceRules`,
`tensionMarkers`, `tabooPhrases`, `sampleLine`을 우선 참고한다. 기존
`archetype`은 씨앗이고, 실제 문장은 다음 계약을 지킨다.

- 텍스트 품질 점검 surface는 `item.text`, `choices[]`, `replies[]`,
  `messages[].text`, `variants[].text`, 변경된 memory/gallery/recollection label이다.
- `choices[]`는 “조사/확인”이 아니라 학범이 어떤 말투·거리·온도로 답할지 보여준다.
- `replies[]`는 보고 문장이 아니라 채팅 말투다. 한 답장은 짧고, route별 호칭/질투/농담이 보여야 한다.
- 장면 본문은 “그날의 기록”, “선택의 무게”, “묘한 긴장감”, “따뜻한 분위기” 같은 범용 요약어를 반복하지 않는다.
- `조사`, `단서`, `수색`, `범인`, `사건` 같은 단어는 로맨스 반응을 일으키는 소품일 때만 쓴다. 선택지 행동으로 쓰지 않는다.
- prose 품질 검사는 `scenarioValidator`가 아니라 `tests/ui-contract.test.mjs` 또는 별도 story-lint script에서 한다.

| Route | Strong voice rule | Must show | Avoid |
| --- | --- | --- | --- |
| 현겸 | 담백한 순애가 우산/소매/자리로 조용히 독점욕을 드러낸다 | 우산 손잡이, 젖은 어깨, 기다렸다는 낮은 말 | 모두에게 같은 순애 문장 |
| 욱현 | 짧고 무심하지만 학범의 습관을 너무 정확히 기억한다 | 접지 않은 노트, 창가 자리, 늦은 답장 | 차갑기만 한 정보 전달 |
| 재성 | 능글 플러팅 뒤, 꺼진 마이크 앞에서 목소리를 낮춘다 | 비공개 멘트, 방송실 유리, 장난 뒤 진심 | 농담으로 감정 무효화 |
| 상원 | 선택을 남기고 싶어 하지만 허락을 묻는 균열을 둔다 | 출입 명단, 지워지지 않는 선, 학범 이름 옆 빈칸 | 폭력적/강제적 얀데레 |
| 상욱 | 먼저 달려가도 학범의 보폭에서 멈춘다 | 달려온 숨, 손목을 멈추는 순간, 같은 보폭 | 무작정 돌진만 반복 |
| 준혁 | 논리와 농담으로 숨다가 계산이 틀릴 때 진심이 드러난다 | 지도 빈칸, 막힌 경로, 계산 밖의 변수 | 확률 말장난만 남발 |
| 도훈 | 장난과 정보값 핑계로 덮지만 행동이 먼저 다정하다 | 영수증 뒷면, 편의점 불빛, 말없이 챙김 | 정보 설명만 계속 |
| 하음 | 박자·숨·손끝 같은 구체 감각으로 부드럽게 가까워진다 | 같은 박자, 피아노 잔향, 기다리는 정적 | 몽환적 표현 남발 |
| 윤호 | “선배” 호칭을 유지하며 예의 뒤에 질투와 기다림을 둔다 | 옥상 난간, 늦게 부르는 선배, 기다리는 자리 | 선배 호칭 누락 |
...
- 리라이트 목표는 노골적 묘사 없는 17금 긴장감이다. 가까운 거리, 질투, 숨 고르기, 손끝의 온기처럼 암시로만 올린다.
