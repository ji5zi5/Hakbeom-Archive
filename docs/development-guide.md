...
- Longform route background 명명(`day6-<route>-study`, `day7-<route>-rain`, `day8-<route>-festival`, `day9-<route>-rumor`)은 500-map production batch의 기본 규칙이다. 2026-05-19 복구 기준으로 Day 6–9 전 route 배경 36개는 직접 생성 PNG + prompt sidecar + `sourceGeneratedImage` provenance가 있으며, `day9-haeum-rumor`, `day9-yunho-rumor`도 직접 생성분으로 복구되어 base background로 되돌리지 않는다. route-depth 배경은 manifest `routeId`와 asset provenance `expansionBatch: route-depth-2026-05` metadata를 유지하고, scene의 story batch 값과 혼동하지 않는다. `derivedFrom`/색보정 파생 배경은 커밋하지 않는다.
