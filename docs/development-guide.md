...
- Longform route background 명명(`day6-<route>-study`, `day7-<route>-rain`, `day8-<route>-festival`, `day9-<route>-rumor`)은 500-map production batch의 기본 규칙이다. 2026-05-19 복구 기준으로 Day 6–8 전체와 Day 9의 7개 route 배경은 직접 생성 PNG + prompt sidecar + `sourceGeneratedImage` provenance가 있다. `day9-haeum-rumor`, `day9-yunho-rumor`는 아직 직접 생성분이 없으므로 기존 accepted base background를 쓴다. `derivedFrom`/색보정 파생 배경은 커밋하지 않는다.
